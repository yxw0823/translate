...
public final class Regexp
{
	...
	{
		System.out.println(getSoftRegexpArray("首映：2015-02-19", ".{2}.{1}(.*)$"));
		// Regexp.isHardRegexpValidate(querys[5], "$\\d{1}");
		System.out.println(isHardRegexpValidate("$1", "\\$\\d{1,2}"));
		/*
		 ...
		 * matcher.getMatch(); // 一个提取html标签属性的例子 String regexpForFontTag = "<\\s*font\\s+([^>]*)\\s*>";
		 * String regexpForFontAttrib = "([a-z]+)\\s*=\\s*\"([^\"]+)\"";
		 * 
		 * String html = " <font face=\"Arial, serif\"
		 * size=\"+2\"color=\"red\">"; System.out.println(regexpForFontTag);
		 * System.out.println(regexpForFontAttrib); System.out.println(html);
		 * 
		 * Pattern p1 = compiler.compile(regexpForFontTag,
		 * Perl5Compiler.CASE_INSENSITIVE_MASK); Pattern p2 =
		 * compiler.compile(regexpForFontAttrib,
		 * Perl5Compiler.CASE_INSENSITIVE_MASK);
		 * System.out.println(matcher.contains(html, p1)); if
		 * (matcher.contains(html, p1)) { MatchResult result =
		 * matcher.getMatch(); System.out.println(result.group(1)); String
		 * attribs = result.group(1);
		 * 
		 * PatternMatcherInput input = new PatternMatcherInput(attribs);
		 * System.out.println(matcher.contains(input, p2)); while
		 * (matcher.contains(input, p2)) { result = matcher.getMatch();
		 ...
		 * MatchResult matchResult = matcher.getMatch(); String http =
		 * "http://www.suncer.com:8080/index.html?login=true"; String
		 * http_regexp = "(\\w+)://([^/:]+)(:\\d*)?([^#\\s]*)"; Pattern p1 =
		 * compiler.compile(http_regexp, Perl5Compiler.CASE_INSENSITIVE_MASK);
		 * System.out.println(matcher.contains(http, p1)); if
		 * (matcher.contains(http, p1)) { MatchResult result =
		 * matcher.getMatch(); System.out.println(result.group(1)); String
		 * attribs = result.group(1);
		 * 
		 * for (int i = 0; i < result.length() && result.group(i) != null; i++) {
		 * System.out.println(i + " : " + result.group(i)); } } } catch
